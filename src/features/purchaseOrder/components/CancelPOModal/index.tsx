...
const CancelPOModal = ({
  ...
  const { fileList, setFileList, cancelFormRef, onSaveCancelPO, isLoading } = useCancelModal({
    hideModal,
    requesterName,
    requesterSite,
    requesterSection,
  ...
  return (
    ...
    >
      <Spin spinning={isFetchingData || isLoading}>
        <CancelPurchaseForm
          isDisabledForm={[PO_STATUS.PO_CANCELING, PO_STATUS.PO_CANCELED].includes(domainStatus)}
          domain={domain}
